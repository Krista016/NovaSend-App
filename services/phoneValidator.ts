...
// Sort codes by length descending to match longest prefix first (e.g. +1242 before +1)
...
export function validateAndNormalizePhoneNumber(phoneNumber: string): ValidationResult {
    if (typeof phoneNumber !== 'string' || !phoneNumber) {
        return { isValid: false, normalizedNumber: null, error: 'Phone number cannot be empty.' };
    }
    ...
    // Case 1: Starts with + (International E.164 format)
    if (cleanedNumber.startsWith('+')) {
        const matchingCode = sortedCountryCodes.find(c => cleanedNumber.startsWith(c.dial_code));
        if (matchingCode) {
            const numberPart = cleanedNumber.substring(matchingCode.dial_code.length);
            // General rule: total length between 8 and 15 digits is common for international numbers
            if (numberPart.length > 0 && cleanedNumber.length >= 8 && cleanedNumber.length <= 15) {
                return { isValid: true, normalizedNumber: cleanedNumber };
            }
        }
        return { isValid: false, normalizedNumber: null, error: 'Invalid international number or country code.' };
    }

    // Case 2: Starts with 0 and is 11 digits long (Indian mobile)
    if (digitsOnly.startsWith('0') && digitsOnly.length === 11) {
        const nationalNumber = digitsOnly.substring(1);
        return { isValid: true, normalizedNumber: `+91${nationalNumber}` };
    }

    // Case 3: Is 10 digits long (Indian mobile)
    if (digitsOnly.length === 10) {
        return { isValid: true, normalizedNumber: `+91${digitsOnly}` };
    }
    ...
    return { 
        ...
        error: 'Number must be 10 digits (India), 11 digits starting with 0 (India), or in international format (e.g., +1...)' 
    };
}
